...
router.post('/repositories', async (req: Request, res: Response) => {
  try {
    ...
    const repo = await GitService.registerRepository(validatedData);
...
router.get('/repositories/:id/status', async (req: Request, res: Response) => {
  try {
    ...
    const status = await GitService.checkRepositoryStatus(objectId);
    res.json(status);
  } catch (error: any) {
    if (error instanceof ZodError) {
      // Safe: error.issues exists
...
router.get('/repositories/:id/commits', async (req: Request, res: Response) => {
  try {
    ...
    const commits = await GitService.fetchCommits(objectId);
    res.json(commits);
  } catch (error: any) {
    if (error instanceof ZodError) {
      // Safe: error.issues exists
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const objectId = new mongoose.Types.ObjectId(repoId);
      const branches = await GitService.getBranches(objectId);
      if (!branches.length) {
        return res
          .status(404)
